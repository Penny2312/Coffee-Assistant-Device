...
void UI::drawDrinkSelect(bool redraw) {
  ...
  if (drink != lastDrink) {
    for (int i = 0; i < 2; i++) {
      uint16_t bg = (i == drink) ? COFFEE_GREEN : COFFEE_BROWN;
      uint16_t fg = (i == drink) ? BLACK : WHITE; 
...
void UI::drawStrengthSelect(bool redraw) {
  ...
  if (drink == ESPRESSO) {
    if (strength != lastStrength) {
      for (int i = 0; i < 2; i++) {
        uint16_t bg = (i == strength) ? COFFEE_GREEN : COFFEE_BROWN;
        uint16_t fg = (i == strength) ? BLACK : WHITE;              
...
void UI::drawDose(bool redraw) {
  ...
  if (doseIndex != lastDoseIndex) {
    for (int i = 0; i < 3; i++) {
      ...
      tft.fillRect(5, 65 + i * 22, 45, 20, selS ? COFFEE_GREEN : COFFEE_BROWN);
      tft.fillRect(74, 65 + i * 22, 45, 20, selD ? COFFEE_GREEN : COFFEE_BROWN);

      tft.setTextSize(2);
      tft.setTextColor(selS ? BLACK : WHITE);
      tft.setCursor(10, 68 + i * 22); tft.print(single[i]); tft.print("g");

      tft.setTextColor(selD ? BLACK : WHITE);
...
void UI::drawGrind(bool redraw) {
  ...
  if (grindSize != lastGrind) {
    ...
    tft.fillRect(0, 65, tft.width(), 30, COFFEE_BROWN);
...
void UI::drawSummary(bool redraw) {
  if (redraw) {
    ...
    printCentered(buf, 45, WHITE, 2); 

    printCentered(currentRatioText(), 65, WHITE, 2);

    snprintf(buf, sizeof(buf), "OUT: %dg", (int)r.targetOutput);
    printCentered(buf, 85, COFFEE_YELLOW, 2);
...
void UI::drawBrewing(bool redraw) {
  ...
  if ((int)(shotTime * 10) != (int)(lastShownTime * 10)) {
    ...
    tft.fillRect(0, 70, tft.width(), 30, COFFEE_BROWN);
    char buf[16];
...
void UI::drawAdvice(bool redraw) {
  if (redraw) {
    ...
    if (lastResult.grindAdjustment < 0) {
      ...
      printCentered("GRIND", 80, WHITE, 2);   // 
      printCentered("FINER", 100, WHITE, 2);  // 
    }
    else if (lastResult.grindAdjustment > 0) {
      printCentered("TOO SLOW", 50, RED, 2);
      printCentered("GRIND", 80, WHITE, 2);   // 
      printCentered("COARSER", 100, WHITE, 2); // 
    }
    else {
